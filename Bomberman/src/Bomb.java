...
public class Bomb {
	...
	public int getNb_J1() {// compte le nombre de bombes dans la  liste Bombs contenant l'owner_id = 1
	...
	public void explose(Board b){
		...
		while (it.hasNext()){//parcours la liste de bombe
			...
			if( bo.getT_explosion() <  System.currentTimeMillis() ){ // le minuteur prend fin
				...
				while(b.isDestructible(bo.Y+i, bo.X) && i<=3){// soit case verte un mur cassable
					if(b.isWallDestructible(bo.Y+i, bo.X) ){
				...
				i=1;
				while(b.isDestructible(bo.Y-i, bo.X) && i<=3){	
					if(b.isWallDestructible(bo.Y-i, bo.X) ){
				...
				i=1;
				while(b.isDestructible(bo.Y, bo.X+i) && i<=3){
					if(b.isWallDestructible(bo.Y, bo.X+i) ){
